export default class MosaicSession {
        ...
        return new Promise((resolve, reject) => {
            self.getCurrentUser()
                ...
                .catch((error) => {
                    const errorMsg =
                        error.message ||
                        "Error getting current Mosaic user. Your authorization may have expired. Make sure you are still logged into Mosaic, and relaunch the project.";
                    reject(errorMsg);
                });
        });
    }
    ...
    // Post analysis to mosaic
    async postAnalysisToMosaic(projectId, analysis) {
        let self = this;
        const response = await fetch(`${self.api}/projects/${projectId}/analyses?client_application_id=21`, {
            method: "POST",
            headers: {
                Authorization: self.authorizationString,
    ...
    promisePostAnalysisToMosaic(projectId, analysis) {
        let self = this;
        return new Promise((resolve, reject) => {
            self.postAnalysisToMosaic(projectId, analysis)
                .then((response) => {
                    resolve(response);
                })
                .catch((error) => {
                    const errorMsg = self.getErrorMessage(error);
                    console.error(`Error posting analysis to Mosaic with project_id ${projectId}`);
                    console.error(errorMsg);
                    reject(`Error posting analysis to Mosaic ${projectId}: ${errorMsg}`);
                });
        });
    }

    async getAnalysisFromMosaic(projectId, analysisId) {
        let self = this;
    ...
    promiseGetAnalysisFromMosaic(projectId, analysisId) {
        let self = this;
        return new Promise((resolve, reject) => {
            self.getAnalysisFromMosaic(projectId, analysisId)
                .then((response) => {
                    resolve(response);
                })
                .catch((error) => {
                    const errorMsg = self.getErrorMessage(error);
                    console.error(`Error getting analysis from Mosaic with project_id ${projectId}`);
                    console.error(errorMsg);
                    reject(`Error getting analysis from Mosaic ${projectId}: ${errorMsg}`);
                });
        });
    }
